...
var OntimizeExportService = (function () {
    ...
    OntimizeExportService.prototype.exportData = function (data, format, entity) {
        var url = this._urlBase + (this.exportPath ? this.exportPath : '') + this.servicePath + '/' + entity + '/' + format;
        var options = {
        ...
        this.httpClient.post(url, body, options).subscribe(function (resp) {
            if (resp && resp.code === Codes.ONTIMIZE_UNAUTHORIZED_CODE) {
                ...
                self.downloadFile(resp.data[0][format + 'Id'], format)
                    .subscribe(function (r) { return _innerObserver.next(r); }, function (e) { return _innerObserver.error(e); }, function () { return _innerObserver.complete(); });
            }
            else {
    ...
    OntimizeExportService.prototype.downloadFile = function (fileId, fileExtension) {
        var url = this._urlBase + (this.downloadPath ? this.downloadPath : '') + this.servicePath + '/' + fileExtension + '/' + fileId;
        var _innerObserver;
        var dataObservable = new Observable(function (observer) { return _innerObserver = observer; }).pipe(share());
        var options = {
            headers: new HttpHeaders({
